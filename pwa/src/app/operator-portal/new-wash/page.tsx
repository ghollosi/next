...
export default function NewWashPage() {
  ...
  // Check if selected vehicle type typically has a trailer
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (!partnersRes.ok || !servicesRes.ok || !pricesRes.ok) {
          if (partnersRes.status === 401 || servicesRes.status === 401 || pricesRes.status === 401) {
        ...
        const [partnersData, servicesData, pricesData] = await Promise.all([
          partnersRes.json(),
          servicesRes.json(),
          pricesRes.json(),
        ]);
