...
export default function NewWashEventPage() {
  ...
  const loadFormData = async () => {
    try {
      ...
      // Load locations, services, and partner companies in parallel
      const [locResponse, partnerResponse] = await Promise.all([
        fetch(`${API_URL}/operator/locations`, { headers }),
        fetch(`${API_URL}/operator/partner-companies`, { headers }),
      ]);
      ...
      if (locResponse.ok) {
        ...
        if (locData.length > 0) {
          ...
          const svcResponse = await fetch(
            `${API_URL}/operator/locations/${locData[0].id}/services`,
            { headers }
          );
          if (svcResponse.ok) {
  ...
  const handleLocationChange = async (locationId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const svcData = await response.json();
        setServices(svcData);
        if (svcData.length > 0) {
          setSelectedServiceId(svcData[0].id);
        }
