...
function RegisterQrContent() {
  const params = useParams();
  const router = useRouter();
  const networkSlug = params.network as string;
  const partnerCode = params.partner as string;

  const [step, setStep] = useState(0); // 0 = loading, 1-3 = form steps
  ...
  }, [networkSlug, partnerCode]);

  const loadRegistrationInfo = async () => {
    try {
      const response = await fetch(
        `${API_URL}/pwa/register-info/${encodeURIComponent(networkSlug)}/${encodeURIComponent(partnerCode)}`
      );

      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.message || 'Hiba történt a regisztrációs adatok betöltésekor');
      }
      ...
      setError(err.message || 'Érvénytelen QR kód vagy a link már nem aktív');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    setError('');
    if (!validateStep3() || !regInfo) return;

    setSubmitting(true);

    const vehicles: VehicleInput[] = [
      { category: 'TRACTOR', plateNumber: tractorPlate.toUpperCase(), plateState: tractorState },
    ];
    ...
    try {
      const response = await fetch(`${API_URL}/pwa/register-qr`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          networkSlug: regInfo.networkSlug,
          partnerCode: regInfo.partnerCompanyCode,
          firstName,
          lastName,
  ...
  const handleResendVerification = async (type: 'EMAIL' | 'PHONE') => {
    if (!registrationResult || !regInfo) return;
    ...
    try {
      const response = await fetch(`${API_URL}/pwa/resend-verification`, {
        ...
        headers: {
          ...
          'x-network-id': regInfo.networkId,
        },
        body: JSON.stringify({
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">Regisztrációs adatok betöltése...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-primary-600 text-white p-6 pb-12">
        <h1 className="text-2xl font-bold mt-2">Sofőr regisztráció</h1>
        {regInfo && (
            ...
            <div className="space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-4">
                <p className="text-sm text-green-800">
                  <strong>Partner cég:</strong> {regInfo?.partnerCompanyName}
                </p>
              </div>
