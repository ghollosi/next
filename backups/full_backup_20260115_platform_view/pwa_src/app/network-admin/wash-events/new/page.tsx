  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm p-6 space-y-6">
        ...
        {/* Plate(s) - depends on locationType */}
        {(() => {
          const selectedLoc = locations.find(l => l.id === selectedLocationId);
          const isCarWash = selectedLoc?.locationType === 'CAR_WASH';

          return (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {isCarWash ? 'Rendszám *' : 'Vontató rendszáma *'}
                </label>
                {isCarWash && (
                  <div className="mb-2 px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full inline-block">
                    Autómosó - csak 1 rendszám
                  </div>
                )}
                <input
                  type="text"
              ...
              {/* Trailer Plate - only for TRUCK_WASH */}
              {!isCarWash && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
